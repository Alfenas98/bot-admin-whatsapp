...
const { pareceMasculino, pareceApresentacao, sortearFrase, jaFoiZoado, marcarComoZoado } = require('./lib/zoeiraNovato');
...
async function startBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    if (configZoeira.zoeiraNovato.ativo && !jaFoiZoado(groupId, senderId)) {
      if (pareceApresentacao(textContent) && pareceMasculino(msg.pushName)) {
        const frase = sortearFrase().replace('@user', `@${senderId.split('@')[0]}`);
