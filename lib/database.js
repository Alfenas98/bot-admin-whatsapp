...
const DEFAULT_GROUP_CONFIG = {
  ...
  // automações agendadas (mensagem fixa, backup, resumo, sorteio, lembretes)
  agendamentos: [],

  // log de auditoria administrativa
  auditoria: { ativo: false, destino: null }, // destino: JID de grupo/privado, ou null = manda no próprio grupo

  // avisa quando nome/descrição do grupo mudam
  alertaMudancaGrupo: false,

  // repostar mensagem entre grupos
  broadcast: { podeEnviar: false, receber: false },

  // zoa homens que mandam a primeira mensagem no grupo (detecção por nome,
  // é uma aproximação, não é 100% precisa)
  zoeiraNovato: { ativo: false, frasesCustom: [] } // se vazio, usa as frases padrão
};
