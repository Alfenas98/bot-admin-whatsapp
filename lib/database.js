...
const DEFAULT_GROUP_CONFIG = {
  ...
  saida: {
    ativo: false,
    mensagem: 'VAI E NÃO VOLTA CORNO(A), @user 👋'
  },
  levelSystem: false,
  autosticker: false,
  ...
  lembretes: [],
  // Relatório
  metrics: { mensagensEnviadas: 0 },
  // Reações automáticas
  reacoesAutomaticas: { ativo: true, custom: {} },
  // Caixa anônima
  caixaAnonima: { ativo: false, historico: [] },
  // Histórico de membros (entrada/saída)
  membros: [],
  // Sistema de cargos
  cargos: { ativo: false, modo: 'automático' }
}
...
function setGroupConfig(groupId, keyPath, value) {
  const config = getGroupConfig(groupId); // garante que o grupo existe e está atualizado
  
  console.log(`[setGroupConfig] Grupo: ${groupId}, Key: ${keyPath}, Value:`, value);
  
  // Garantir que o caminho existe antes de setar
  const keys = keyPath.split('.');
  let atual = db.get(['groups', groupId]).value();
  
  for (let i = 0; i < keys.length - 1; i++) {
    if (!atual[keys[i]] || typeof atual[keys[i]] !== 'object') {
      atual[keys[i]] = {};
    }
    atual = atual[keys[i]];
  }
  
  db.set(['groups', groupId, ...keys], value).write();
  
  // Verificar se salvou corretamente
  const salvo = db.get(['groups', groupId, ...keys]).value();
  console.log(`[setGroupConfig] Valor salvo:`, salvo);
