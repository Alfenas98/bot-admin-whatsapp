...
// Importação síncrona no topo
let getPatente, getTopUsuarios, db;
try {
  const xp = require('./xp');
  getPatente = xp.getPatente;
  getTopUsuarios = xp.getTopUsuarios;
  db = require('./database').db;
} catch (e) {
  console.error('[cron] Erro ao importar módulos:', e.message);
}

function iniciarCronTop10(sock) {
  if (!getPatente || !getTopUsuarios || !db) {
    console.error('[cron] Módulos não carregados, cron desabilitado');
    return;
  }
