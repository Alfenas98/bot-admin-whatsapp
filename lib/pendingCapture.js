...
function aguardarFigurinha( groupId, senderId, jogoId, duracaoMs = DURACAO_PADRAO_MS ) {
  pendentes.set(`${groupId}:${senderId}`, {
    jogoId,
    expiraEm: Date.now() + duracaoMs,
  });
}

/** * Consome (remove) a espera pendente, se existir e ainda não tiver expirado. * Retorna o jogoId, ou null se não havia nada esperando ou já expirou. */
