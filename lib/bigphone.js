...
function iniciarBigphone(sock, groupId) {
  ...
  // Continuar enviando toques a cada 1-6 horas
  setInterval(() => {
    if (!jogoAtivo) return;
    
    // Delay aleatório entre 1-6 horas (3600000ms a 21600000ms)
    const delay = Math.floor(Math.random() * 18000000) + 3600000;
  ...
  }, 21600000); // Verificar a cada 6 horas
}
...
async function enviarToque(sock, groupId) {
  ...
  // Verificar se passou tempo suficiente desde o último toque (mínimo 30 minutos)
  if (Date.now() - ultimoToque < 1800000) return;
  ...
  // Timeout: se ninguém responder em 10 minutos, desistir
  setTimeout(() => {
    if (aguardandoResposta) {
      aguardandoResposta = false;
      console.log('[bigphone] Ninguém atendeu a ligação');
    }
  }, 600000); // 10 minutos
}
