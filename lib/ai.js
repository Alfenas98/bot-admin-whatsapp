...
Se não souber algo, seja honesto.`,
  ...
  AUTO_RESPONDER: {
    enabled: true,
    // Palavras que ativam resposta automática (só bot e ia)
    gatilhos: ['bot', 'ia']
  }
};
...
function isAutoResponderTrigger(text) {
  ...
  // Só ativa se contiver "bot" ou "ia" como palavra isolada
  if (/\bbot\b/.test(lower)) return true;
  if (/\bia\b/.test(lower)) return true;
