...
Se não souber algo, seja honesto.`,
  ...
  AUTO_RESPONDER: {
    enabled: true,
    // Palavras que ativam resposta automática
    gatilhos: [
      'bot', 'oi bot', 'eae bot', 'salve bot', 'bot ajuda',
      'que horas são', 'que dia é hoje', 'piada', 'curiosidade',
      'dica', 'sugestão', 'resumo', 'explica'
    ],
    // Probabilidade de responder a menções aleatórias
    randomChance: 0.1
  }
};
...
function isAutoResponderTrigger(text) {
  ...
  // Verificar gatilhos
  for (const gatilho of CONFIG.AUTO_RESPONDER.gatilhos) {
    if (lower.includes(gatilho.toLowerCase())) {
      return true;
    }
  }
  
  // Verificar se menciona o bot
  if (lower.includes('@bot') || lower.includes('bot,')) {
    return true;
  }
