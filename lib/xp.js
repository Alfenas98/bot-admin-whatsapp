...
/**
 * Obtém cargo baseado no nível
 */
function getCargo(nivel) {
  const cargos = [
    { min: 1, max: 3, cargo: '🆕 Novato' },
    { min: 4, max: 6, cargo: '🟢 Ativo' },
    { min: 7, max: 10, cargo: '🔵 Membro Fiel' },
    { min: 11, max: 15, cargo: '🟡 Veterano' },
    { min: 16, max: 20, cargo: '🟠 Elite' },
    { min: 21, max: 30, cargo: '🔴 Mestre' },
    { min: 31, max: 50, cargo: '👑 Lenda' },
    { min: 51, max: Infinity, cargo: '💎 Diamond' }
  ];
  
  for (const c of cargos) {
    if (nivel >= c.min && nivel <= c.max) {
      return c.cargo;
    }
  }
  return '💎 Diamond';
