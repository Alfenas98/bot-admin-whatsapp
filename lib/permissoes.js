...
function verificarPermissao(groupId, senderId, messageType, textContent) {
  ...
  console.log(`[permissoes] Verificando permissão para ${senderId} no grupo ${groupId}`);
  console.log(`[permissoes] Config cargos:`, config.cargos);
  
  // Se o sistema de cargos não está ativo, não verificar
  if (!config.cargos?.ativo) {
    console.log(`[permissoes] Sistema de cargos não ativo, ignorando`);
